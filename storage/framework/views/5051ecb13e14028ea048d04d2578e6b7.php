            ...
            <div class="tab-content orders-filters">
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            <?php if($statuses): ?>
                                <?php
                                    // Define the statuses to include, ensuring proper capitalization
                                    $desiredStatuses = ['Closed', 'Just created', 'Updated'];
                        
                                    // Create a map for the desired statuses to ensure proper capitalization
                                    $statusMap = array_combine(
                                        array_map('strtolower', $desiredStatuses),
                                        $desiredStatuses
                                    );
                        
                                    // Filter and map statuses using the statusMap to ensure proper capitalization
                                    $filteredStatuses = array_filter($statuses, function($status) use ($statusMap) {
                                        $lowercaseStatus = strtolower($status);
                                        return isset($statusMap[$lowercaseStatus]);
                                    });
                        
                                    $formattedStatuses = [];
                                    foreach ($filteredStatuses as $status) {
                                        $lowercaseStatus = strtolower($status);
                                        if (isset($statusMap[$lowercaseStatus])) {
                                            $formattedStatuses[$statusMap[$lowercaseStatus]] = $statusMap[$lowercaseStatus];
                                        }
                                    }
                            ...
                            <?php echo $__env->make('partials.select', [
                                ...
                                'data' => \App\Enums\PaymentType::forSelect(),
                                'required' => false,
                                'value' => ''
                            ...
                            <div class="d-none">
                                <?php echo $__env->make('partials.select', [
                                    ...
                                    'id'=>"status_id",
                                    'placeholder'=>"Select status",
                                    'data'=> ['unpaid'=> ("Just created"), 'paid' => __("Paid")],
                                    'required' => false, 
                                    'value'=>''
                                ], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
                            </div>
                        ...
                        <?php if(\Spatie\Permission\PermissionServiceProvider::bladeMethodWrapper('hasRole', 'admin|owner')): ?>
                        <div class="col-md-3">
                            <div class="form-group">
    ...
    <div class="table-responsive">
        <table class="table align-items-center">
            ...
            <?php elseif(config('app.isqrsaas')): ?>
                <?php echo $__env->make('orders.partials.orderdisplay_local', ['hideAction'=>true], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
            <?php else: ?>
...
</div><?php /**PATH /var/www/html/resources/views/orders/partials/ordercard.blade.php ENDPATH**/ ?>