...
class Order extends Model
{
    ...
    protected function situation(): Attribute
    {
        return Attribute::get(function () {
            try {
                ...
                return PaymentType::tryFrom($this->payment_status);
            } catch (\Throwable $th) {
                return PaymentType::UNPAID;
            }
        });
    ...
    protected function remainingPrice(): Attribute
    {
        return Attribute::get(function () {
            $orderPrice = $this->order_price + $this->vatvalue;
            foreach ($this->transactions()->where('is_paid', true)->get() as $transaction) {
                $orderPrice -= $transaction->amount_with_tax;
            }
    ...
    {
        return Attribute::get(fn () => $this->order_price + $this->vatvalue + $this->fee_value);
    }

    protected function fullPrice(): Attribute
    {
        return Attribute::get(fn () => $this->order_price + $this->vatvalue + $this->fee_value + $this->tip);
    }
