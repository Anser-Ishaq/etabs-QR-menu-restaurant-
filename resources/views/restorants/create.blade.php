                        ...
                        <div class="pl-lg-4">
                            <form method="post" action="{{ route('admin.restaurants.store') }}">
                                ...
                                <h6 class="heading-small text-muted mb-4">{{ __('Owner information') }}</h6>
                                    ...
                                    @endif
                                    {{-- @include('partials.input',['type'=>"text", 'name'=>'Owner Phone','id'=>"phone_owner",'placeholder'=>"Owner Phone here",'required'=>true,'value'=>""]) --}}
                                    {{-- <input  type="text" id="cloneWith" name="cloneWith" value="Message here" /> --}}



                                    <hr />
                                    <h6 class="heading-small text-muted mb-4">{{ __('Add Restaurant to a grpup ') }}</h6>
                                    
                                        <div class="form-group{{ $errors->has('restaurant_group') ? ' has-danger' : '' }}">
                                            <label class="form-control-label" for="restaurant_group">{{ __('Group Name') }}</label>
                                            <select name="restaurant_group" class="form-control form-control-alternative{{ $errors->has('restaurant_group') ? ' is-invalid' : '' }}">
                                                <option value="" class="form-control">{{ __('Select Group') }}</option>
                                                @foreach (\App\Models\Group::pluck('name') as $group)
                                                    <option class="form-control">{{ $group }}</option>
                                                @endforeach
                                            </select>
                                            {{-- <input type="text" name="restaurant_group" id="restaurant_group" class="form-control form-control-alternative{{ $errors->has('restaurant_group') ? ' is-invalid' : '' }}"  placeholder="{{ __('Enter Group Name') }} ..." value="" required autofocus> --}}
                                            @if ($errors->has('restaurant_group'))
                                                <span class="invalid-feedback" role="alert">
                                                    <strong>{{ $errors->first('restaurant_group') }}</strong>
                                                </span>
                                            @endif
                                        </div>
                                        <div class="form-group{{ $errors->has('description') ? ' has-danger' : '' }}">
                                            <label class="form-control-label" for="description">{{ __('Any Message') }}</label>
                                            <input type="text" name="description" id="description" class="form-control form-control-alternative{{ $errors->has('description') ? ' is-invalid' : '' }}" placeholder="{{ __('Enter Group info') }} ..." value=""    >
