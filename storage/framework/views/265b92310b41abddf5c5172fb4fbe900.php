    ...
    <div class="container-fluid">
        <?php if(!config('settings.hide_project_branding')||(!isset($restorant))): ?>
          <a class="navbar-brand mr-lg-5" href="/">
            <img style="height: 65px" class="theProjectLogo" src="<?php echo e(asset('images/logo.jpg')); ?>">
          </a>
        <?php else: ?>
        ...
        <div class="navbar-collapse collapse" id="navbar_global">
          ...
          <ul class="navbar-nav align-items-lg-center ml-lg-auto">
            <?php if(isset($restorant)): ?>
                ...
                <?php echo $__env->yieldContent('addiitional_button_3'); ?>
                <?php if(config('app.isqrsaas')): ?>
                  
                  <?php if(\Request::route()->getName() != "cart.checkout"&&!config('settings.is_whatsapp_ordering_mode') && !$restorant->getConfig('disable_callwaiter', 0) && strlen(config('broadcasting.connections.pusher.app_id')) > 2 && strlen(config('broadcasting.connections.pusher.key')) > 2 && strlen(config('broadcasting.connections.pusher.secret')) > 2): ?>
                  ...
                  <?php if($restorant->getConfig('clients_enable','false')!='false'): ?>
            ...
            <li class="mobile-menu">
              ...
              <?php if(isset($restorant)): ?>
                ...
                <?php if(config('app.isqrsaas')): ?>
                  <?php if(\Request::route()->getName() != "cart.checkout" && !$restorant->getConfig('disable_callwaiter', 0) && strlen(config('broadcasting.connections.pusher.app_id')) > 2 && strlen(config('broadcasting.connections.pusher.key')) > 2 && strlen(config('broadcasting.connections.pusher.secret')) > 2&&!config('settings.is_pos_cloud_mode')): ?>
...
<?php /**PATH /var/www/html/resources/views/layouts/menu/top_justlogo.blade.php ENDPATH**/ ?>